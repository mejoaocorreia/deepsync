...
Version is one input, not the compatibility result. The adapter also verifies a canonical-home and nonce-bound isolation marker, readable profile composition, packed archive safety, full artifact identity, public manifest and DSH binding schemas, target and capability declarations, package installation, exact artifact selection, bundle selection, Loader execution, correlated `deepsync.health/v1` evidence, rollback config hash, and rollback profile-tree digest.
